...
const TintImages = memo(({ opacity }: { opacity: number }) => {
  return (
    <div className="relative h-[60vw] max-h-[400px] w-full rounded-2xl border border-neutral-300 bg-white lg:h-screen overflow-hidden">
      {/* Base "Car" Represented by a colored div */}
      <div className="absolute inset-0 bg-slate-200 flex items-center justify-center font-bold text-slate-400">
        Car Interior View
      </div>

      {/* Tint Overlay (opacity controlled by slider) */}
      <MotionDiv 
        animate={{ opacity }} 
